...
Instead of the naive prisoner game, imagine the following: You have a reporter reward. Anyone can report a sealed value by committing a fixed amount of liquidity. If after reveal everyone agrees (unanimity), they split the reward. If anyone disagrees, all the liquidity becomes the reward for the next round, which has the same fixed liquidity commitment to participate. Naturally, we avoid needing to decide who is in the participant set and deal with Sybils in the strong sense by making identity irrelevant.
