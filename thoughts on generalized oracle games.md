...
It may not be possible to have receipt-freeness onchain. There are also challenges with preventing selective non-reveal from becoming a strategic weapon. Maybe there is some cryptographic technique allowing full participant set reveal after some period of time, like with VDFs. In order to preserve the lack of knowledge of the other's report in the classic prisoner game, we want a scheme where, during a live window, a participant cannot provide binding evidence of their true report, but after a delay the protocol can verify unanimity versus disagreement.
