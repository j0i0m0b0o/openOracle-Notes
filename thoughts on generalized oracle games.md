...
If delay in the context of griefing external dependents (not strictly continuation farming) seems like an Achilles' heel, the reporter liquidity requirement can be made to grow each round by a smaller % than the reward pot grows, instead of being fixed, capping out at some number. Long delay is equivalent to playing a near-exponential continuation game. Frivolous delay cost is a function of oracle game parameters and external notional size either way.
...
A weird possibility is that a participant may rationally add a report, or even two disagreeing reports, simply because doing so directly increases continuation value. It may not take much to spark a kind of continuation value “singularity.” Also, if many people, or a large amount of capital, depend on the oracle output, expectations of future game value may revise sharply upward. Once we have the spark, we can get to the late game, where it seems brutally difficult to get a lie to finalize. Even without continuation value expectations, why would it be reasonable to assume an important oracle question would have thin participation in the early game? And if participation is not thin in the early game, it becomes more likely to resolve to either ground truth or extend into the late game, where it seems quite difficult to get the oracle to settle on a lie.

The game is terrible in the attrition sense. For many, it isn't worth it to start playing, but then once you start playing, it isn't worth it to stop, and the amounts can be catastrophic. It is unfortunately structurally forced on the mechanism when you try to port the prisoner coordination game on-chain in a robust way. It is possible nobody wants to play.
